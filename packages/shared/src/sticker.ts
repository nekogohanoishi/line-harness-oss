...
export function createStickerMessageContent(message: StickerSource): StickerMessageContent | null {
  ...
  const stickerResourceType = toOptionalString(
    message.stickerResourceType ?? message.sticker_resource_type,
  );
  ...
  try {
    ...
    if (
      ...
      return {
        ...
        fallback: typeof parsed.fallback === 'string' && parsed.fallback.trim()
          ? parsed.fallback
          : STICKER_FALLBACK,
      };
    }
